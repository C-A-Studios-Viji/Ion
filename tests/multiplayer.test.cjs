...
test('five-digit host and guest exchange room state and live actions over a data connection',async()=>{
  ...
  const {PartyLink,partyPeerId,validPartyCode}=module.exports;
  ...
  host.onConnect=()=>host.send({type:'hello',state:{room:25},pose:{x:0,y:0,z:1,yaw:0,light:true,dead:false,room:25}});
